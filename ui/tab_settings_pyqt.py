...
class SettingsPage(QScrollArea):
    ...
    def setup_ui(self):
        ...
        card_title = QLabel("🎨 " + t("settings_appearance"))
        ...
        lang_label.setObjectName("cardLabel")
        ...
        self.lang_combo.setFocusPolicy(Qt.FocusPolicy.StrongFocus)  # Evitar cambio con scroll
        ...
        theme_label.setObjectName("cardLabel")
        self.theme_combo = NoScrollComboBox()
        self.theme_combo.addItems([t("settings_dark"), t("settings_light")])
        self.theme_combo.setCurrentText(t("settings_dark") if self.settings.get('theme', 'dark') == 'dark' else t("settings_light"))
        self.theme_combo.currentTextChanged.connect(self.on_theme_changed)
        self.theme_combo.setFocusPolicy(Qt.FocusPolicy.StrongFocus)  # Evitar cambio con scroll
        ...
        printer_title = QLabel("🖨️ " + t("settings_default_printer"))
        printer_title.setObjectName("cardTitle")
        printer_layout.addWidget(printer_title)

        printer_desc = QLabel(t("settings_default_printer_desc"))
        printer_desc.setObjectName("cardDescription")
        ...
        self.printer_combo.setFocusPolicy(Qt.FocusPolicy.StrongFocus)  # Evitar cambio con scroll
        ...
        print_title = QLabel("🔧 " + t("settings_print_settings"))
        print_title.setObjectName("cardTitle")
        print_layout.addWidget(print_title)

        wait_desc = QLabel(t("settings_wait_time"))
        wait_desc.setObjectName("cardDescription")
        ...
        self.wait_label = QLabel(f"{self.wait_slider.value()} {t('settings_seconds')}")
        self.wait_label.setObjectName("cardDescription")
        ...
        license_title = QLabel("🔑 " + t("settings_license"))
        ...
        if self.parent_window and hasattr(self.parent_window, 'license_manager'):
            ...
            if license_info and license_info['is_valid']:
                ...
                license_text = f"✅ {t('settings_license_active')}\n\n{t('license_expires')}: {license_info['expires_at'].strftime('%Y-%m-%d')}\n{t('license_days_remaining')}: {days}"
            else:
                color = "#DC2626"
                license_text = f"❌ {t('settings_license_inactive')}"
        else:
            color = "#DC2626"
            license_text = f"❌ {t('settings_license_inactive')}"
        ...
        btn_license = QPushButton(t("settings_renew_license"))
        ...
        update_title = QLabel("🔄 " + t("settings_updates"))
        update_title.setObjectName("cardTitle")
        update_layout.addWidget(update_title)

        version_label = QLabel(f"{t('settings_current_version')} {getattr(self.parent_window, 'VERSION', '1.0.0')}")
        version_label.setObjectName("cardLabel")
        update_layout.addWidget(version_label)

        btn_update = QPushButton(t("settings_check_updates"))
        ...
        btn_reset = QPushButton(t("settings_reset"))
    ...
    def on_theme_changed(self, text):
        is_dark = (text == t("settings_dark") or text == "Dark")  # Support both translated and English
        self.settings['theme'] = 'dark' if is_dark else 'light'
        if self.parent_window:
            self.parent_window.switch_theme(is_dark)

    def on_wait_time_changed(self, value):
        self.wait_label.setText(f"{value} {t('settings_seconds')}")
